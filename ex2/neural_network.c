...
float run_neural_network(struct neural_network *nn, bool is_learning)
{
	...
	for (size_t output_index = 0; output_index < OUTPUT_COUNT; output_index++)
	{
		...
		{
			//fprintf(stderr, "\tInput = %"PRId8", weight = %.3f\n", nn->input_layer[input_index], nn->weights[input_index][output_index]);
		...
		//fprintf(stderr, "- Potentiel = %.4f, désiré = %"PRId8"\n", potential, nn->desired_output);
		float output_error = nn->desired_output - potential;
...
int neural_network_train(struct neural_network *nn)
{
	...
	for (uint32_t iteration = 0; total_error > 0.00001f; iteration++)
	{
		total_error = 0.0f;
